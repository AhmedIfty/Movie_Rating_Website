...
if(isset($_POST['add_students'])){
    ...
    if(($title == "" || empty($title)) || ($year == "" || empty($year)) || ($season == "" || empty($season)) || ($typee == "" || empty($typee))){
        header('location: admin_series.php?message=Any of the field is empty!');
    }


    else{

        $query = "insert into `content` (`title` , `year_released`, `type`, `seasons`, `trailer`) 
            values ('$title', '$year', '$typee', '$season', '$trailer')";
        ...
        if(!$result){
            ...
            header('location:admin_series.php?insert_msg= Your data has been added successfully');
        }
